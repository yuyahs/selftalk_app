  ...
  describe "POST/create" do
    context "名前が空欄でメッセージは入力されている場合" do
      it "エラーレスポンス400を返す" do
        expect{
          post "/api/contacts", params: {inquiry: {name: "", message: "message"}}
    ...
    context "名前が入力されていてでメッセージは空の場合" do
      it "エラーレスポンス400を返す" do
        expect{
          post "/api/contacts", params: {inquiry: {name: "user", message: ""}}
        }.to change(Inquiry, :count).by(0)
        expect(ActionMailer::Base.deliveries.size).to eq(0)
        expect(response).to have_http_status "400"
      end
    ...
    context "名前もメッセージも入力されている場合" do
      it "成功レスポンス200を返す" do
        expect{
          post "/api/contacts", params: {inquiry: {name: "user", message: "message"}}
